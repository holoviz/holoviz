...
* [holoviz-dev](https://github.com/holoviz-dev/) hosts two main types of repositories:
    * Packages that support maintaining the core HoloViz packages, including `nbsite`, `pyct` and `autover`. These support packages were developed when no alternative, or satisfying alternative, was available at the time the group needed them.
...
* [holoviz-dev](https://github.com/holoviz-dev/)
    ...
    * [status-dashboard](https://github.com/holoviz-dev/status-dashboard): Status Dashboard for HoloViz Projects
    ...
    * Development versions of the various holoviz project docs
...
* [coc-committee](https://github.com/orgs/holoviz/teams/coc-committee): HoloViz Organization Code of Conduct Committee members
* [holoviz-dev](https://github.com/orgs/holoviz/teams/holoviz-dev): Team that will manage the HoloViz contents
* [hvplot-dev](https://github.com/orgs/holoviz/teams/hvplot-dev): hvPlot maintainers
* [panel-maintainers](https://github.com/orgs/holoviz/teams/panel-maintainers): The team of maintainers for the Panel project
...
* `pixi.toml` is where the environments and tasks are declared.
* `pixi.lock` (ignored by `git`)  is the lock file that is automatically generated by `pixi` when running commands that need to create an environment; it contains the exact versions of the dependencies used in the environment.
* `scripts` is a directory that contains scripts (Python, bash, etc.) called by some `pixi` tasks.
...
* Install `pixi` if you haven't already, instructions are available [here](https://pixi.sh/latest/#installation).
...
* *Linters and Code Formatters*: These tools automatically check your Python code for errors and ensure it adheres to style guidelines. Most HoloViz projects use [Ruff](https://docs.astral.sh/ruff) together with [pre-commit](https://pre-commit.com/) to run these checks on every commit. You can manually run linting with `pixi run lint` and install the pre-commit hook using `pre-commit run lint-install`.
* *Unit tests*: the HoloViz projects rely on [pytest](https://docs.pytest.org/) to run their unit tests, sometimes with some additional pytest plugins. The `pixi run test-unit` command will run these tests.
...
1. Tag a commit with a version (e.g., `git tag -m "Version 1.9.6 alpha1" v1.9.6a1 main`)
